...
class ZhiShouData {
  ...
  factory ZhiShouData.fromJson(Map<String, dynamic> json) {
    return ZhiShouData(
      ...
      id: json['id'] as String, // 解析 type 字段
    );
  }
...
class RecordBean {
  ...
  void addDataByDate(String inputDate, String type, String state, String num,
  ...
  // 计算当月各个state类型的总金额
  Map<String, double> calculateStateTotals() {
    Map<String, double> stateTotals = {};
    double totalAmount = 0.0; // 当月总金额

    // 遍历当月的每日数据
    monthlyData.forEach((day, monthData) {
      for (var zhiShouData in monthData.zhiShouList) {
        String state = zhiShouData.state;
  ...
  // 获取state总金额列表
  Future<List<Map<String, dynamic>>?> getStateTotalList(String month, String type) async{
